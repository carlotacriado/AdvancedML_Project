#!/usr/bin/env python
# coding: utf-8

import torch
from torch.utils.data import Dataset, DataLoader
import numpy as np
import random
from trains.train_baseline import get_baseline_dataloaders
from sklearn.model_selection import train_test_split
from Utils.globals import *
from Utils.utils import seed_worker, set_all_seeds

# --- Wrapper del Dataset ---
class MappedSubset(Dataset):
    """
    Dataset virtual que:
    1. Filtra las imagenes permitidas (segun train_labels/val_labels).
    2. Traduce la etiqueta global a una etiqueta local (0..N) para la loss function.
    """
    def __init__(self, dataset, indices, label_map):
        self.dataset = dataset
        self.indices = indices
        self.label_map = label_map

    def __len__(self):
        return len(self.indices)

    def __getitem__(self, idx):
        real_idx = self.indices[idx]
        image, global_label = self.dataset[real_idx]
        
        # Traducimos la etiqueta (Pokedex ID o Family ID)
        target_label = self.label_map[global_label]
        
        return image, target_label

# --- Función Principal ---
def get_baseline_dataloaders(dataset, train_labels, val_labels, task_mode='pokedex', batch_size=64, seed=SEED):
    """
    Genera dataloaders para el modelo Baseline respetando el split del Meta-Learning.
    
    Args:
        dataset: Instancia de PokemonMetaDataset
        train_labels: Lista de IDs permitidos en Train (output de get_structured_splits)
        val_labels: Lista de IDs permitidos en Val
        task_mode: 'pokedex' (clasificar por especie) u 'oak' (clasificar por familia)
        seed: Semilla para el generador del dataloader
    """
    
    print(f"\n--- Configurando Baseline Dataloaders (Task: {task_mode.upper()}) ---")

    # 1. Configurar Generador
    g = torch.Generator()
    g.manual_seed(seed)

    # 2. Crear Mapa de Etiquetas (Global -> Local 0..N)
    active_species = sorted(list(set(train_labels)))
    label_map = {}
    
    if task_mode == 'pokedex':
        # Cada especie es una clase distinta (0..59)
        for i, species_id in enumerate(active_species):
            label_map[species_id] = i
        num_classes = len(active_species)
        print(f"Baseline Pokedex: {num_classes} clases (Especies).")

    elif task_mode == 'oak':
        # Agrupar especies por Familia
        family_to_int = {}
        current_fam_idx = 0
        
        for species_id in active_species:
            fam_str = dataset.idx_to_family[species_id]
            if fam_str not in family_to_int:
                family_to_int[fam_str] = current_fam_idx
                current_fam_idx += 1
            
            label_map[species_id] = family_to_int[fam_str]
            
        num_classes = current_fam_idx
        print(f"Baseline Oak: {num_classes} clases (Familias).")
    
    else:
        raise ValueError("task_mode debe ser 'pokedex' u 'oak'")

    # 3. Aplanar indices de imagenes (Convertir lista de Clases -> Lista de Fotos)
    all_train_indices = []
    for label in train_labels:
        if label in dataset.indices_by_label:
            all_train_indices.extend(dataset.indices_by_label[label])
    
    train_idx, val_idx = train_test_split(
        all_train_indices, 
        test_size=0.2, 
        random_state=seed, 
        shuffle=True
    )

    # 4. Crear Datasets y Loaders
    train_ds = MappedSubset(dataset, train_idx, label_map)
    val_ds = MappedSubset(dataset, val_idx, label_map)

    train_loader = DataLoader(
        train_ds,
        batch_size=batch_size,
        shuffle=True, # Shuffle activado, controlado por generator=g
        num_workers=2,
        worker_init_fn=seed_worker,
        generator=g
    )
    
    val_loader = DataLoader(
        val_ds,
        batch_size=batch_size,
        shuffle=False,
        num_workers=2,
        worker_init_fn=seed_worker,
        generator=g
    )

    return train_loader, val_loader, num_classes