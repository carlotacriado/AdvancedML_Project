...
class FamilyMappedDataset(Dataset):
    """
    Wrapper that converts Species Classification -> Family Classification.
    
    Logic:
    1. Input: Image Index
    2. Lookup: Species ID (e.g., 25 Pikachu)
    3. Map: Family ID (e.g., 10 Pikachu-Line)
    4. Output: Local Class Label (0..N)
    """
    ...
    def __getitem__(self, idx):
        ...
        # Get image and original Species ID
        image, species_id = self.dataset[real_idx]
        
        # 1. Map Species -> Evolutionary Family
        fam_id = self.species_to_family_map.get(species_id)
        
        # 2. Map Family ID -> Local Training Label (0...N_Classes)
        # If family is not in the map (e.g., belongs to Test split), return -100 (Ignored by CrossEntropy)
...
def get_evolution_dataloaders(batch_size, seed):
    """
    Sets up DataLoaders where the target is the Evolutionary Family, not the specific Species.
    """
    print("\n--- Setting up Dataloaders by FAMILIES ---")
    
    
    # 1. Load CSV to analyze family structures
    df = pd.read_csv(CSV_PATH)
    
    # Create Mapping: Species_ID (dex_number) -> Family_ID
    species_to_family = pd.Series(df.family_id.values, index=df['dex_number'].values).to_dict()
    
    # 2. Split Unique Families (Not Images)
    # We want the model to learn to recognize "families", so we split by unique family IDs.
    unique_families = df['family_id'].unique()
    print(f"Total unique families: {len(unique_families)}")
    ...
    print(f"Split: {len(train_fams)} Train | {len(val_fams)} Val | {len(test_fams)} Test Families")
    
    # 3. Create Label Map (Global Family ID -> 0..N)
    # Only map Training families, as the classifier head is fixed size.
    family_to_label = {fam_id: i for i, fam_id in enumerate(train_fams)}
    num_classes = len(train_fams)
    
    # 4. Define Transforms
    # Strong augmentation for training to force learning invariant features
    ...
    # 5. Filter Images belonging to Training Families
    all_train_indices = []
    
    # full_ds_train.indices_by_label maps: { Species_ID : [List of image indices] }
    for species_id, indices_list in full_ds_train.indices_by_label.items():
        fam_id = species_to_family.get(species_id)
        
        # Only include image if its family is in the Train Split
        if fam_id in train_fams:
            all_train_indices.extend(indices_list)
            
    print(f"Total Images in Training Set: {len(all_train_indices)}")

    # 6. Internal Split: Train/Val (80/20) on the filtered images
    # Note: We use the same families for validation here to monitor overfitting on the training classes.
