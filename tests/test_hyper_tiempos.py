...
def save_filtered_confusion_matrix(all_true, all_preds, dataset, save_path):
    ...
    top_names = [dataset.idx_to_name.get(l, f"ID_{l}")[:10] for l in top_labels] 
...
def main():
    ...
    for N_WAY in N_WAY_LIST:
        for N_SHOT in N_SHOT_LIST:
            ...
            # --- WANDB INIT PARA ESTA CONFIGURACIÓN ---
            run_name = f"Eval_{N_WAY}way_{N_SHOT}shot"
            wandb.init(
                project=WANDB_PROJECT,
                name=run_name,
                group="Evaluation_Benchmarks",
            ...
            try:
                ...
                wandb.finish() # Cerrar run si falla
                continue
            ...
            with torch.no_grad():
                for batch_idx, (images, global_labels) in enumerate(tqdm(test_loader, desc=f"Episodios ({N_WAY}w{N_SHOT}s)")):
                    ...
                    latencies.append(timer.get_time_ms())
                    # ---------------------------------------------
            ...
            print(f"   -> Acc: {mean_acc*100:.2f}% | Latency: {mean_latency:.2f}ms (±{std_latency:.2f})")
            ...
            summary_results.append({
                ...
                "N_Shot": N_SHOT,
                "Mean_Acc": mean_acc,
                "Mean_Latency_ms": mean_latency, # Added to CSV
                "Std_Latency_ms": std_latency,   # Added to CSV
                "CI_95": ci95,
