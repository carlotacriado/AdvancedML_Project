...
class CudaTimer:
    ...
    def stop(self):
        if self.device.type == 'cuda':
            ...
            torch.cuda.synchronize() # <--- CLAVE: Espera a la GPU
    ...
    for n_way in N_WAYS_LIST:
        for k_shot in K_SHOTS_LIST:
            try:
                ...
                accuracies = []
                adapt_times_ms = [] # List to store time per episode
                ...
                mean_time = np.mean(adapt_times_ms) / 1000.0  # Convert ms to seconds
